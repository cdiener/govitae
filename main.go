...
func build_latex(cv Resume, name string, tpath string) error {
    fs := t_tmp.FuncMap{"join": strings.Join}
    t := t_tmp.Must(t_tmp.New("template.tex").Delims("#(", ")#").
        Funcs(fs).ParseFiles(path.Join(tpath, "template.tex")))
    ...
    if len(cv.Publications) > 0 {
        t = t_tmp.Must(t_tmp.New("template.bib").Delims("#(", ")#").
            Funcs(fs).ParseFiles(path.Join(tpath,"template.bib")))
...
func main() {
    ...
    err = build_latex(cv, name, TEMPLATE_PATH)
    check(err)
